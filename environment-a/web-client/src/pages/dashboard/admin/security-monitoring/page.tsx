...
export function AdminSecurityMonitoringPage(): JSX.Element {
    ...
    return (
        <div className="space-y-6" data-testid="security-monitor-page">
            ...
            <section className="grid items-start gap-6 xl:grid-cols-[minmax(0,1.25fr)_minmax(20rem,0.75fr)]"><SecurityEventList rows={monitoring.rows} loading={monitoring.eventsLoading} page={monitoring.page} totalPages={monitoring.totalPages} hasMore={monitoring.hasMore} selectedEvent={monitoring.selectedEvent} onSelect={monitoring.selectEvent} onPrevious={() => monitoring.setPage(Math.max(monitoring.page - 1, 0))} onNext={() => monitoring.setPage(monitoring.page + 1)} onPageChange={monitoring.setPage} /><EventEvidence event={monitoring.selectedEvent} runEvents={monitoring.selectedRunEvents} loading={monitoring.runLoading} /></section>
        </div>
    );
}
