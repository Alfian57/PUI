...
export function EventEvidence({ event, runEvents, loading }: EventEvidenceProps): JSX.Element {
    if (!event) {
        return (
            <aside className="flex max-h-[min(42rem,calc(100dvh-2rem))] min-h-0 flex-col rounded-[1.75rem] bg-white p-5 shadow-soft ring-1 ring-brand-line/70" data-testid="security-event-evidence">
    ...
    return (
        <aside className="flex max-h-[min(42rem,calc(100dvh-2rem))] min-h-0 flex-col overflow-hidden rounded-[1.75rem] bg-white p-5 shadow-soft ring-1 ring-brand-line/70" data-testid="security-event-evidence">
            <div className="flex shrink-0 items-start justify-between gap-3">
                <div className="min-w-0">
                    <p className="font-display text-[11px] uppercase tracking-[0.2em] text-brand-steel">Bukti event</p>
                    <h2 className="mt-1 break-words font-display text-xl font-semibold text-brand-logoBlue">{event.title || eventMeta(event).label}</h2>
                </div>
                <span className={clsx("rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide", outcomeTone(event.outcome))}>
                    {outcomeLabel(event.outcome)}
                </span>
            </div>
            {event.run_id ? <p className="mt-2 shrink-0 break-all font-mono text-xs text-brand-steel">run_id: {event.run_id}</p> : null}
            {loading ? (
                <div className="mt-5 h-24 shrink-0 animate-pulse rounded-2xl bg-brand-sky" />
            ) : (
