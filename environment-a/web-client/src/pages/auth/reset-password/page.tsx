...
import { confirmPasswordReset } from "@/pages/auth/_api/authApi";
...
export function ResetPasswordPage(): JSX.Element {
    ...
    async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
        ...
        try {
            ...
            navigate(ROUTES.auth.login, { replace: true, state: { resetSuccess: true } });
        } catch (cause) {
        ...
        <main className="min-h-[100dvh] overflow-x-hidden bg-brand-sky text-brand-ink">
            ...
            >
                ...
                <p className="mt-5 text-center text-sm text-brand-steel">
                    <Link className={`inline-flex items-center gap-2 ${authLinkClass}`} to={token ? ROUTES.auth.login : ROUTES.auth.forgotPassword}>
